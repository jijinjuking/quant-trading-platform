# 市场数据服务 (market-data) - 架构设计

## 📋 服务概述

### 服务名称
市场数据服务 (Market Data Service)

### 服务端口
8083

### 服务职责
- 实时市场数据接入 (WebSocket)
- 多交易所连接管理 (Binance/OKX/Huobi)
- 数据处理与标准化
- 数据分发与缓存
- 连接质量监控

## 🏗️ 服务架构

### 内部架构图
```
services/market-data/
│
├── src/
│   │
│   ├── main.rs                 # 服务入口，启动HTTP服务器和WebSocket客户端
│   │
│   ├── state.rs                # 应用状态管理，持有所有组件的Arc引用
│   │
│   ├── config/                 # 配置管理
│   │   ├── mod.rs              # 配置结构体定义
│   │   └── settings.rs         # 配置加载逻辑
│   │
│   ├── handlers/               # HTTP接口层
│   │   ├── mod.rs              # 路由注册
│   │   ├── market_data.rs      # 行情查询接口
│   │   ├── klines.rs           # K线数据接口
│   │   ├── tickers.rs          # 价格接口
│   │   └── health.rs           # 健康检查接口
│   │
│   ├── connectors/             # 交易所连接器
│   │   ├── mod.rs              # 连接器管理
│   │   ├── binance.rs          # Binance交易所连接
│   │   ├── okx.rs              # OKX交易所连接
│   │   ├── huobi.rs            # Huobi交易所连接
│   │   └── exchange_trait.rs   # 交易所接口定义
│   │
│   ├── processors/             # 数据处理器
│   │   ├── mod.rs              # 处理器管理
│   │   ├── kline_processor.rs  # K线数据处理
│   │   ├── tick_processor.rs   # Tick数据处理
│   │   └── orderbook_processor.rs # 订单簿处理
│   │
│   ├── storage/                # 数据存储层
│   │   ├── mod.rs              # 存储接口
│   │   ├── redis_store.rs      # Redis缓存
│   │   └── clickhouse_store.rs # ClickHouse历史数据
│   │
│   ├── models/                 # 数据模型
│   │   ├── mod.rs              # 模型定义
│   │   ├── kline.rs            # K线数据模型
│   │   ├── ticker.rs           # 价格模型
│   │   ├── orderbook.rs        # 订单簿模型
│   │   └── trade.rs            # 交易模型
│   │
│   ├── services/               # 业务逻辑层
│   │   ├── mod.rs              # 服务管理
│   │   ├── data_service.rs     # 数据服务
│   │   ├── subscription_service.rs # 订阅服务
│   │   └── metrics_service.rs  # 指标服务
│   │
│   └── utils/                  # 工具函数
│       ├── mod.rs
│       ├── crypto_utils.rs     # 加密工具
│       └── time_utils.rs       # 时间工具
│
└── Cargo.toml                  # 依赖声明
```

## 🔄 数据流向

### 数据流入 (Inbound)
```
交易所 WebSocket
    ↓ (原始数据)
connectors/
    ↓ (标准化数据)
processors/
    ↓ (处理后数据)
storage/
    ↓ (缓存/持久化)
handlers/ (提供HTTP API)
```

### 数据流出 (Outbound)
```
HTTP客户端 (前端/其他服务)
    ↓
handlers/ (接收请求)
    ↓
storage/ (从缓存/数据库获取)
    ↓
HTTP响应
```

## 📡 API接口设计

### REST API
```http
GET  /api/v1/market/tickers          # 获取实时价格
GET  /api/v1/market/klines           # 获取K线数据
GET  /api/v1/market/orderbook        # 获取订单簿
GET  /api/v1/market/trades           # 获取交易记录
GET  /api/v1/market/symbols          # 获取交易对列表
GET  /health                         # 健康检查
GET  /metrics                        # 监控指标
```

### WebSocket API
```json
{
  "type": "subscribe/unsubscribe",
  "channels": ["kline_1m", "ticker", "orderbook"],
  "symbols": ["BTCUSDT", "ETHUSDT"]
}
```

## 🗄️ 数据模型

### 核心数据结构
```rust
// K线数据模型
pub struct Kline {
    pub symbol: String,
    pub open_time: i64,
    pub open: Decimal,
    pub high: Decimal,
    pub low: Decimal,
    pub close: Decimal,
    pub volume: Decimal,
    pub close_time: i64,
    pub quote_asset_volume: Decimal,
    pub number_of_trades: u64,
}

// 订单簿模型
pub struct OrderBook {
    pub symbol: String,
    pub bids: Vec<(Decimal, Decimal)>, // (价格, 数量)
    pub asks: Vec<(Decimal, Decimal)>,
    pub timestamp: i64,
}

// 交易数据模型
pub struct Trade {
    pub symbol: String,
    pub price: Decimal,
    pub quantity: Decimal,
    pub trade_time: i64,
    pub is_buyer_maker: bool,
}
```

## 🔧 技术实现要点

### 连接管理
- **SOCKS5代理**: 通过SOCKS5代理连接交易所
- **TLS加密**: 使用TLS加密连接
- **连接池**: 维护多个连接以提高可靠性
- **自动重连**: 实现指数退避重连机制

### 数据处理
- **实时处理**: 毫秒级数据处理
- **数据标准化**: 统一不同交易所的数据格式
- **异常数据过滤**: 过滤异常价格和成交量
- **延迟监控**: 实时监控数据延迟

### 性能优化
- **缓存策略**: Redis缓存热点数据
- **批量处理**: 批量处理数据以提高性能
- **异步处理**: 使用Tokio异步运行时
- **内存管理**: 优化内存使用，避免内存泄漏

## 📊 监控指标

### 性能指标
- 连接延迟
- 数据处理延迟
- 消息吞吐量
- 内存使用率

### 业务指标
- 数据完整性
- 连接成功率
- 订阅成功率
- 错误率

## 🔐 安全措施

- **认证**: API密钥认证
- **限流**: 请求频率限制
- **日志**: 详细的访问日志
- **加密**: 数据传输加密

## 🚀 部署配置

### 环境变量
```
MARKET_DATA_PORT=8083
REDIS_URL=redis://localhost:6379
CLICKHOUSE_URL=http://localhost:8123
BINANCE_WSS_ENDPOINT=wss://stream.binance.com:9443/ws
OKX_WSS_ENDPOINT=wss://real.okx.com:8443/ws
HOT_RELOAD=false
```

### Docker配置
- 多阶段构建
- 最小化镜像
- 资源限制

## 🧪 测试策略

### 单元测试
- 数据模型测试
- 处理器逻辑测试
- 工具函数测试

### 集成测试
- 端到端数据流测试
- 连接器功能测试
- API接口测试

### 性能测试
- 压力测试
- 负载测试
- 稳定性测试