//! # 合约布林带策略 (Futures Bollinger Bands Strategy)
//!
//! 基于布林带指标的合约突破策略。
//! 支持杠杆、双向持仓。
...
/// 合约布林带策略配置
#[derive(Debug, Clone)]
pub struct FuturesBollingerConfig {
    /// 周期
    pub period: usize,
    /// 标准差倍数
...
impl Default for FuturesBollingerConfig {
    fn default() -> Self {
        Self {
            period: 20,
...
/// 合约布林带策略状态
#[derive(Debug, Clone)]
pub struct FuturesBollingerState {
    /// 价格历史
...
impl Default for FuturesBollingerState {
    fn default() -> Self {
        Self::new()
    }
}
...
/// 合约布林带策略
pub struct FuturesBollingerStrategy {
    meta: StrategyMeta,
    config: FuturesBollingerConfig,
    state: FuturesBollingerState,
}

impl FuturesBollingerStrategy {
    /// 创建合约布林带策略实例
    pub fn new(
        instance_id: Uuid,
        symbol: String,
        ...
        Self {
            meta: StrategyMeta {
                instance_id,
                strategy_type: "futures_bollinger".to_string(),
                market_type: market,
                symbol,
                is_active: false,
            },
            config,
            state: FuturesBollingerState::new(),
        }
    }
...
impl Strategy for FuturesBollingerStrategy {
    ...
    fn reset(&mut self) {
        self.state = FuturesBollingerState::new();
    }
}
