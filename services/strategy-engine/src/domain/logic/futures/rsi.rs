...
impl Default for FuturesRsiConfig {
    fn default() -> Self {
        Self {
            period: 14,
...
impl Default for FuturesRsiState {
    fn default() -> Self {
        Self::new()
    }
}
...
/// 合约RSI策略
pub struct FuturesRsiStrategy {
    meta: StrategyMeta,
    config: FuturesRsiConfig,
    state: FuturesRsiState,
}

impl FuturesRsiStrategy {
    /// 创建合约RSI策略实例
    pub fn new(
        instance_id: Uuid,
        symbol: String,
        ...
        Self {
            meta: StrategyMeta {
                instance_id,
                strategy_type: "futures_rsi".to_string(),
                market_type: market,
                symbol,
                is_active: false,
            },
            config,
            state: FuturesRsiState::new(),
        }
    }
...
impl Strategy for FuturesRsiStrategy {
    ...
    fn reset(&mut self) {
        self.state = FuturesRsiState::new();
    }
}
